...
# ╔═╡ b4a70b26-030a-11f0-072c-231c03cac311
begin
	using NNlib, Lux
	using Optim
	using LinearAlgebra, Random, Optimisers, Printf, Zygote
	using MakieMaestro, LazyGrids
	using MakieMaestro.Themes, MakieMaestro.Units
end

# ╔═╡ ef280a06-361d-4723-a7fc-3c570724478b
begin
	width!(307u"pt")
	hwratio!(0.65)
	figure_dir!("../src/figs/")
...
# ╔═╡ 55667b67-70f3-4281-a8c2-a155334fd49c
global PLOTS = Dict();

# ╔═╡ b463fc86-8f6d-4386-b2c9-f590cbe22880
with_backend, with_backend! = MakieMaestro.with_backend(PLOTS);

# ╔═╡ 83a6069e-3dfb-4fc9-920d-50fdc9d9e897
begin
	data_x = [
...
# ╔═╡ fd29e278-ef1b-4339-b5be-5cf2982ae342
const gate_activations = [(relu, name=L"\mathrm{ReLU}"), (gelu, name=L"\mathrm{GeLU}"), (mish, name=L"\mathrm{Mish}")]

# ╔═╡ 41580ce7-f36f-4a8a-b9af-9f565568c208
const math_activations = [(sigmoid, name=L"\sigma"), (tanh, name=L"\mathrm{tanh}"),]

# ╔═╡ 7d078b2b-6a12-41fc-ae4a-b844ac4134c8
function fig_activations(activations=math_activations, range=-5 .. 5, name=L("Activation functions"))
    f = Figure()
    a = Axis(f[1, 1]; title=name)
...
# ╔═╡ 2bef3ac0-9419-4474-a7e6-a143c674df3b
with_backend(fig_activations, WGLMakie)

# ╔═╡ 984c9383-9c1a-4cc7-8cce-5ba9853a6892
savefig(fig_activations, "math_activations")

# ╔═╡ 40b43e64-f47f-488c-aba2-455e77cbd973
with_backend(fig_activations, WGLMakie, gate_activations, -1..1, L("Gate Activations"))

# ╔═╡ 0dea0ba1-3c35-448a-b32b-922ea15147c1
savefig(() -> fig_activations(gate_activations, -1..1, L("Gate Activations")), "gate_activations")

# ╔═╡ 29593afe-5a72-4dd0-a387-348f3674305b
...
# ╔═╡ c8dd68b8-ff4f-4e30-9c63-818e42c32367
with_backend(fig_ReLU_variants, WGLMakie)

# ╔═╡ 059ba09b-84a0-40dd-af28-632442f7abc9
savefig(fig_ReLU_variants, "ReLU_variants")

# ╔═╡ 60ccf754-2afb-4bac-a04f-e4d0625e6a6f
...
# ╔═╡ 428442e4-3dcd-4bfd-8cf7-3c3e5c5553a6


# ╔═╡ 0026c817-6104-4aa0-a87d-05bf304f110c
...
# ╔═╡ b1f5370a-27fb-4e38-b428-8a766f0e789b
with_backend(fig_scatter_data, WGLMakie)

# ╔═╡ ed8e8bca-25e2-45aa-8cf9-805b73f3ae16
savefig(fig_scatter_data, "2d_classification_data")

# ╔═╡ 3fb80e89-d3b0-47e3-aa3b-09d50cbeeff6
labels(ys) =
    map(ys) do y
        y[1] > y[2] ? 0 : 1
    end

# ╔═╡ 47248fd4-4e37-462f-9895-dbd3b8a600c5
label_values(ys) =
    map(ys) do y
        y[1] > y[2] ? y[1] : y[2]
    end

# ╔═╡ 5a3e693b-f479-4d62-a840-e5f4655b77a0
function fig_NN_decisions!(a, func)
    x1_range = range(0, 1, 1000)
    x2_range = range(0, 1, 1000)
    x1s, x2s = ndgrid(x1_range, x2_range)
    ls = labels(func.(x1s, x2s))
    l_values = label_values(func.(x1s, x2s))
    heatmap!(a, x1_range, x2_range, map(ls, l_values) do l, v
            l == 0 ? missing : v
        end, colormap=:blues, alpha=0.5)
    heatmap!(a, x1_range, x2_range, map(ls, l_values) do l, v
            l == 1 ? missing : v
        end, colormap=:reds, alpha=0.5)
end

# ╔═╡ d04fff13-d726-4a39-bb80-c8b8a22f072e
function NN_2_3_2_join_params(W2, b2, W3, b3, W4, b4)
...
# ╔═╡ cb1dcd33-7398-462d-a3b5-c5a922b21ae5
function NN_2_3_2_split_params(Pval)
...
# ╔═╡ e698070c-1182-454d-80af-172bdc98b65f
function NN_2_3_2_forward(x; Pval, activation=NNlib.sigmoid)
    W2, b2, W3, b3, W4, b4 = NN_2_3_2_split_params(Pval)
...
# ╔═╡ a2947b1e-60d3-47cc-8c2e-eae500a03f0f
NN_2_3_2_forward(x1, x2; kwargs...) = NN_2_3_2_forward([x1, x2]; kwargs...)

# ╔═╡ 8766a91c-d808-436a-887c-5fe448d25473
...
# ╔═╡ aa5be06c-b642-4559-9492-059b18989e82
...
# ╔═╡ 50bae083-b5f7-4ac2-a432-61f9da7b2d21
# with_backend(fig_nelder_mead_fit, GLMakie)

# ╔═╡ 87cc070b-5dff-4b55-b3d8-534e515efa39
# savefig(fig_nelder_mead_fit, "nelder_mead_fits")
...
manifest_format = "2.0"
project_hash = "f6f9a9b12db4c761780227010b40806bfbd5a54e"
...
git-tree-sha1 = "5ca6df11fa46915dd2ce09a244d6e2367e2c3793"
...
git-tree-sha1 = "b1bb7f8718117221f4125d11c0c0fb6dd5ab2c0c"
